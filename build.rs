...
const MODULE_PATH: &[&str] = &["meta_signal_cloud", "authority"];

fn main() {
    let crate_root = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("manifest dir set"));
    let source_path = crate_root.join("schema/authority.ethos");
    let rust_path = crate_root.join("src/schema/authority/generated.rs");

    println!("cargo:rerun-if-changed=schema/authority.ethos");
    println!("cargo:rerun-if-changed=src/bootstrap_manifest.rs");
    println!("cargo:rerun-if-changed=src/schema/authority/generated.rs");

    let source = fs::read_to_string(&source_path).expect("read authority Interface source");
    ...
    .expect("assemble authority-approved meta cloud Interface transaction");
    let rust = rust_logos();
    let type_paths = AuthorityRustTypePaths::new();

    BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
        .generate()
        .expect("project meta cloud authority Interface from the verified transaction")
        .write_or_check("META_SIGNAL_CLOUD_UPDATE_INTERFACE_ARTIFACTS")
        .expect("checked meta cloud authority Interface projections are fresh");
    CargoEthosSourceMetadata::new("meta-signal-cloud")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("meta cloud authority bootstrap catalog is complete")
}
...
struct AuthorityRustTypePaths(BTreeMap<VocabularyEncodedId, RustTypePath>);

impl AuthorityRustTypePaths {
    fn new() -> Self {
        Self(BTreeMap::from([(
...
impl RustTypePathResolver for AuthorityRustTypePaths {
